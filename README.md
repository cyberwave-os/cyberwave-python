...
# or the live robot, then dispatch through backend control.
rover = cw.twin("unitree/go2")
cw.affect("simulation")   # or cw.affect("live") for the physical robot
rover.control.drive_forward(speed=0.25, duration_ms=1000)
rover.control.stop_velocity()
...
rover = cw.twin("unitree/go2")
rover.control.forward(1.0)              # body-frame relative navigation
rover.control.backward(0.5)
rover.control.drive_forward(speed=0.25, duration_ms=1000)  # velocity policy
rover.control.turn_left(angular=0.4, duration_ms=750)
rover.control.stop_velocity()
# Aerial twins use the same backend control handle:
# drone.control.up(1.0)
...
`move()` and `rotate()` are deprecated. Use `twin.control.forward()` /
`twin.control.backward()` for backend-dispatched relative movement,
`twin.control.drive_forward()` / `twin.control.set_velocity()` for
backend-resolved default locomotion policies. Pass `simulation_backend` to select
a simulation route such as MuJoCo, or pass a typed `policy_ref` only when you
want to override the backend default. SDK velocity commands use the
`locomotion.velocity_command.v1` payload contract, whose canonical schema is
tracked at `contracts/locomotion.velocity_command.v1.schema.json`. Import
`normalize_locomotion_velocity_command()` and
`LOCOMOTION_VELOCITY_COMMAND_REQUIRED_FIELDS` from
`cyberwave.locomotion_contracts` when building custom emitters or runtime
adapters that need the same validation and clamping behavior as the edge and
MuJoCo drivers.
Tools that need to inspect the backend-resolved setup can call
`cw.assets.get_controller_setup(asset_uuid)` for configured controller
bindings, the primary preview controller key, runtime policies, runtime options,
typed `policy_ref` entries, typed `runtime_target` entries, adapters, contracts,
and artifact manifests.
The response is typed as `AssetControllerSetupView`; import
`PolicyRefPayload` and `ControlRuntimeTargetPayload` when storing or inspecting
typed policy references and runtime targets.
Use the legacy `move_forward()`,
`move_backward()`, `turn_left()`, and `turn_right()` helpers only for direct MQTT
teleop, or `edit_rotation()` to directly set orientation.

### Centered Placement (MuJoCo-style scenes) — *stub*

Cyberwave stores twin pose in **URDF link-origin notation**: `position_*` locates the asset's link origin in world coordinates and `scale_*` rescales the asset-local mesh bounds. For primitive shapes whose STL origin is at a corner or face (rather than the geometric center), authoring in that notation forces users to subtract a per-axis "local center" offset every time — and that offset depends on the asset *and* on the per-axis scale.

The SDK ships a small set of helpers that translate **MuJoCo-style centered authoring** (`center + dimensions + rotation`) into the same `position_*` / `scale_*` / `rotation_*` Cyberwave fields. Backend semantics are unchanged.

```python
from cyberwave import Cyberwave
from cyberwave.placement import (
    GENERIC_CUBE_BOUNDS,
    compute_centered_placement,
)

cw = Cyberwave()
scene = cw.get_scene("acme/envs/sdc-demo")

# Author by geometric center + world dimensions (MuJoCo / IsaacLab style).
support_box = scene.add_twin_centered(
...
# Author a 4 cm red cube resting on top of the support box.
red_cube = scene.add_twin_centered(
    "cyberwave/generic_cube",
    name="red_cube",
    center=(0.375, 0.0, 0.74),
    dimensions=(0.04, 0.04, 0.04),
    asset_bounds=GENERIC_CUBE_BOUNDS,
)

# Move an existing twin by its new world center (keeps current scale and rotation).
red_cube.edit_centered_pose(
    center=(0.40, 0.05, 0.74),
    asset_bounds=GENERIC_CUBE_BOUNDS,
)

# Or just compute the equivalent link-origin fields without calling the backend.
placement = compute_centered_placement(
    center=(0.525, 0.0, 0.36),
    dimensions=(0.70, 0.80, 0.72),
    asset_bounds=GENERIC_CUBE_BOUNDS,
)
# placement.position == (0.525, -0.03824, 0.0)
# placement.scale    == (0.7, 0.8, 0.72)
```

When to use which API:

| API                                 | Notation                | Use when                                      |
| ----------------------------------- | ----------------------- | --------------------------------------------- |
| `scene.add_twin` / `edit_position`  | URDF / link-origin      | Asset's link origin already matches authoring intent (most robots). |
| `scene.add_twin_centered` / `edit_centered_pose` | MuJoCo / geometric center | Primitive shapes whose STL origin is at a corner or face (e.g. the generic cube), or when porting from a MuJoCo / IsaacLab scene. |

`asset_bounds` is the asset-local axis-aligned bounding box `((min_x, min_y, min_z), (max_x, max_y, max_z))`. The SDK exports `GENERIC_CUBE_BOUNDS` for the bundled `cyberwave/generic_cube` asset; for other assets pass the bounds explicitly (e.g. from the asset's mesh or universal schema).
...
`joints.get()` / `get_joints()` return a **live view** — a dict-like object that always reflects the latest state, so you read it again instead of calling `get()` again:

```python
state = arm.joints.get()          # live view
print(state["shoulder_pan"])      # current value, re-reads each access
# react to every update:
state = arm.joints.get(after_update_callback=lambda s: print(s))
state.stop()                      # cancel the callback
sub = arm.joints.on_update(lambda s: print(s)); sub.cancel()  # standalone
```
...
**Command catalog** introspection and invocation both live on `twin.commands`; register or read full MQTT + Zenoh catalogs on `twin.driver`:

```python
dog = cw.twin("unitree/go2")
print(dog.commands.get_supported_commands())
schema = dog.commands.get_schema()  # MQTT catalog (topics, commands.specs, …)
dog.driver.set_schema("cw-driver.yml")  # or MyDriver.get_manifest() — persists + re-binds
dog.commands.move_forward(linear_x=0.3, duration=0.5, rate_hz=10)
# dog.commands.sit_down()  # catalog-only, single publish
```

→ [examples/commands_catalog.py](examples/commands_catalog.py). Full transports/Zenoh: `twin.driver.get_schemas()`, `twin.describe()`.
...
rover = cw.twin("unitree/go2")
rover.control.drive_forward(speed=0.25, duration_ms=1000)  # MuJoCo policy if configured

# Switch command helpers to the live robot
cw.affect("live")
rover.control.drive_forward(speed=0.25, duration_ms=1000)  # requires live readiness

# Explicit overrides use the backend PolicyRef shape.
rover.control.set_velocity(
    linear=0.2,
    simulation_backend="mujoco",
    policy_ref={
        "kind": "catalog_seed_id",
        "value": "controller:go2-mujoco-velocity-policy:v1",
    },
)

# You can still use the legacy direct MQTT helpers when needed
...
## RL Tasks (`cyberwave.rl_tasks.RLTaskClient`) — *stub*

The SDK ships a thin REST wrapper for the RL Tasks API and helpers for the post-training half of the workflow (upload a trained checkpoint, download it back, and stage it for local MuJoCo play via `cyberwave-rl`):
...
# Push a freshly-trained checkpoint back to the RL task
rl.upload_checkpoint(
    task_uuid="<task-uuid>",
    path="cyberwave-rl/logs/<task>/<run>/checkpoints/best_agent.pt",
    name="best_agent",
)

# Later, pull it down and stage it for cyberwave-rl's PlayConfig
checkpoint = rl.list_checkpoints("<task-uuid>")[0]
downloaded = rl.download_checkpoint(
    task_uuid="<task-uuid>",
...
The same surface covers scene-entity authoring, the strict task spec, the Actions / Observations / RL config tabs, and the backend-dispatched inference path; see `cyberwave.rl_tasks` for the full method list.
...
| Area | Use | Notes |
| --- | --- | --- |
| **Driver catalog** | `twin.driver` | `set_schema`, `get_schemas`, `get_mqtt_schema` / `get_zenoh_schema`, `get_supported_topics` / `channels` / `transports`, `get_command_specs`. |
| **Commands (catalog)** | `twin.commands` | `get_schema()`, `get_supported_commands()`, and `twin.commands.<name>(**kwargs)` for invocation. Per-command `continuous` spec triggers burst publish; else capability delegate or single MQTT publish. |
...
| **Discovery** | `twin.describe()` | Top-level `driver` (catalogs, transports, `command_specs`) and `commands` (bound methods, `command_routing`, `publish`). Summary in `interfaces`. |
...
| **Catalog introspection** | `twin.commands.get_schema()`, `get_supported_commands()`; `twin.driver.get_schemas()` for MQTT + Zenoh | Command surface on `twin.commands`; transports, topics, Zenoh on `twin.driver`. |
| **Register manifest** | `twin.driver.set_schema(driver.manifest)` or `set_schema("cw-driver.yml")` | Persists `metadata["mqtt"]` / `metadata["zenoh"]` and re-binds `twin.commands.<name>`. |
| **Export `cw-driver.yml`** | `MyDriver.get_manifest(path="cw-driver.yml")` | Classmethod — no twin or `run()` required. |
| **Edge driver (`BaseDriver`)** | `examples/fake_imu_driver.py` | `define_interface` → `manifest` / `cw-driver.yml`, lifecycle, `driver.run()`. [Docs](https://docs.cyberwave.com/edge/drivers/writing-compatible-drivers). |
...
| `twin.commands.set_schema()` | **Removed** | `twin.driver.set_schema()` (persists metadata and re-binds `twin.commands.<name>`) |
| `twin.driver.get_supported_commands()` only | **Shortcut added** | Prefer `twin.commands.get_supported_commands()` when exploring the command catalog; `twin.driver` still works |
