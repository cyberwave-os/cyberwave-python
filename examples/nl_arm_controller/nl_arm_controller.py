...
def run_self_check() -> int:
    print("─" * 64)
    print("  NL → PiPER Controller — environment self-check")
    print("─" * 64)
...
class _NullRobot:
    """Stand-in robot so --dry-run can exercise plans and teleop offline."""

    class _Joints:
        def set(self, joint_name: str, position: float, degrees: bool = True) -> None:
            return None
...
def _dry_run_executor():
    ...
    return MotionExecutor(_NullRobot(), dry_run=True)


def _enter_teleop(executor) -> None:
    from teleop import run_keyboard_teleop  # noqa: WPS433

    try:
        run_keyboard_teleop(executor)
    except KeyboardInterrupt:
        print("\n  (teleop interrupted)")


def run_agent(dry_run: bool, voice: bool, vision: bool, keys_only: bool = False) -> int:
    if not keys_only and not ANTHROPIC_API_KEY:
