...
"""
Example: Register a camera sensor and stream frames with a simple model runtime in backend.
...
- Model runtime hooks in backend (ai_tasks.py) can process frames and emit events.
"""
...
def main():
    ...
    # Send a single frame (backend model runtime can process and append events)
