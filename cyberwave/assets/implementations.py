...
Updated to use device specifications as single source of truth.
"""
...
# Import device specs for enhanced functionality
try:
...
class DjiTello(FlyingRobot):
    """
    DJI Tello educational drone with device spec integration
    
    Enhanced with device specifications for consistent capabilities,
    setup wizards, and twin creation support.
    """
    ...
    def __init__(self, ip: str = "192.168.10.1", **kwargs):
        ...
        # Get device specification if available
        self._device_spec = None
        if DEVICE_SPECS_AVAILABLE:
            self._device_spec = DeviceSpecRegistry.get("dji/tello")
            
            if self._device_spec:
                # Use spec for capabilities and specs
            ...
            # Fallback when device specs not available
            self._capabilities.extend(['flip', 'stream_video'])
            self._specs.update({
                'max_altitude': 10,
                'max_speed': 8,
    ...
    # ------------------------------------------------------------------
    # Twin backed convenience helpers
    # ------------------------------------------------------------------

    def _require_twin(self, twin: Optional[object]) -> object:
        resolved = twin or getattr(self, "twin", None)
        if resolved is None:
            raise RuntimeError(
                "A twin instance is required. Call cyberwave.twin('dji/tello') first "
                "and pass it in, or attach it to this asset via asset.twin = ..."
            )
        return resolved

    def takeoff(self, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "takeoff"):
            return controller.takeoff()
        raise RuntimeError("Twin does not expose a takeoff capability.")

    def land(self, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "land"):
            return controller.land()
        raise RuntimeError("Twin does not expose a land capability.")

    def emergency_stop(self, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "emergency_stop"):
            return controller.emergency_stop()
        raise RuntimeError("Twin does not expose an emergency stop capability.")

    def start_video_stream(self, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "start_video_stream"):
            return controller.start_video_stream()
        raise RuntimeError("Twin does not expose video streaming capabilities.")

    def stop_video_stream(self, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "stop_video_stream"):
            return controller.stop_video_stream()
        raise RuntimeError("Twin does not expose video streaming capabilities.")

    def navigate_to(self, x_cm: float, y_cm: float, z_cm: float, *, speed_cm_s: float = 50.0, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "navigate_to"):
            return controller.navigate_to(x_cm, y_cm, z_cm, speed_cm_s)
        raise RuntimeError("Twin does not expose navigation helpers.")
    
    async def create_twin(self, client, project_id: str, environment_id: str = None, **config):
        """
        Create digital twin with automatic device registration
        
        Uses device specification for proper device registration and twin creation.
        """
        if not DEVICE_SPECS_AVAILABLE or not self._device_spec:
            raise RuntimeError("Device specifications not available for twin creation")
        
        # Register device using SDK with spec information
        device_config = {
            "name": config.get("name", self.name or self._device_spec.name),
            "device_type": self._device_spec.category,
            "capabilities": [cap.name for cap in self._device_spec.capabilities],
            "metadata": {
                **self._device_spec.to_dict(),
                "ip": self.ip,
                **config
            },
        ...
        device = await client.devices.create(**device_config)
        
        # Create twin if environment specified
        if environment_id:
            twin_config = {
                "device_id": device["id"],
                "environment_id": environment_id,
                "asset_registry_id": self._device_spec.id,
                "name": f"{device['name']} (Twin)",
                **config
            }
            twin = await client.twins.create(twin_config)
            return twin
        
        return device
    
    def get_supported_commands(self) -> List[str]:
        """Get all supported commands from device spec"""
        if self._device_spec:
            return self._device_spec.get_all_commands()
        return ["takeoff", "land", "move", "flip"]  # Fallback
    
    def get_setup_wizard(self) -> Optional[List[Dict[str, Any]]]:
        """Get setup wizard configuration from device spec"""
        if self._device_spec:
            return [
                {
                    "name": field.name,
                ...
                for field in self._device_spec.setup_wizard
            ]
        return None
...
class BostonDynamicsSpot(GroundRobot):
    ...
    def _require_twin(self, twin: Optional[object]) -> object:
        ...
        if resolved is None:
            raise RuntimeError(
                "A twin instance is required. Call cyberwave.twin('boston-dynamics/spot') "
                "first and pass it in, or attach it to this asset via asset.twin = ..."
            )
        return resolved

    def walk(self, speed_m_s: float = 0.5, *, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "walk"):
            return controller.walk(speed_m_s)
        raise RuntimeError("Twin does not expose walking controls.")

    def turn(self, yaw_degrees: float, *, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "turn"):
            return controller.turn(yaw_degrees)
        raise RuntimeError("Twin does not expose turn controls.")

    def sit(self, *, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "sit"):
            return controller.sit()
        raise RuntimeError("Twin does not expose sit controls.")

    def stand(self, *, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "stand"):
            return controller.stand()
        raise RuntimeError("Twin does not expose stand controls.")

    def navigate_to(self, target: Mapping[str, float], *, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "navigate_to"):
            return controller.navigate_to(target)
        raise RuntimeError("Twin does not expose navigation helpers.")

    def get_camera_feed(self, *, twin: Optional[object] = None) -> Any:
        controller = self._require_twin(twin)
        if hasattr(controller, "get_camera_feed"):
            return controller.get_camera_feed()
        raise RuntimeError("Twin does not expose perception helpers.")
