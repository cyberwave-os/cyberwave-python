...
import numpy as np
import dataclasses
# Legacy level import removed - levels are now environments
import warnings
...
class Client:
    ...
    def __init__(
        ...
        Args:
            base_url: The base URL of the CyberWave backend. If provided,
                overrides environment setting.
            environment: Predefined environment (LOCAL, DEV, QA, STAGING, PROD).
        ...
        """
        # Determine backend URL using new environment system
        if base_url is None:
            base_url = get_backend_url(environment=environment)
        
        logger.info(f"🌐 CyberWave Client initialized")
        logger.info(f"   Environment: {environment.value if environment else 'custom/env-var'}")
        logger.info(f"   Backend URL: {base_url}")
        
        # Ensure API prefix is present and no trailing slashes
        api_base_url = base_url.rstrip('/')  # Remove any trailing slashes from base_url
        if API_VERSION_PREFIX not in api_base_url:
            api_base_url = api_base_url + API_VERSION_PREFIX
        
        # Ensure no trailing slash on final API base URL
        ...
        self.base_url = api_base_url  # Store base URL for URL generation
        self._access_token: Optional[str] = None
        self._refresh_token: Optional[str] = None
        self._use_token_cache = use_token_cache
        self._session_info: Dict[str, Any] = {}

        # Initialize specialized APIs (following the MissionsAPI pattern)
        ...
        from .ml import MLAPI
        from .assets_api import AssetsAPI
        from .events import EventsAPI
        
        # Create unified async HTTP client for all APIs
        self._http = AsyncHttpClient(
            base_url=api_base_url,
            access_token_getter=lambda: self._access_token,
            timeout=timeout
        )
        
        # Delegate to specialized APIs (proper segregation of competence)
        self.projects = ProjectsAPI(self._http)
        self.environments = EnvironmentsAPI(self._http)
        self.assets = AssetsAPI(self._http)
        self.twins = TwinsAPI(self._http, assets_api=self.assets)
        self.nodes = NodesAPI(self._http)
        self.devices = DevicesAPI(self._http)
        self.missions = MissionsAPI(self._http)
        self.runs = RunsAPI(self._http)
        self.sensors = SensorsAPI(self._http)
        self.teleop = TeleopAPI(self._http)
        self.workflows = WorkflowsAPI(self._http)
        self.agents = AgentsAPI(self._http)
        self.ml = MLAPI(self._http)
    ...
    def _save_token_to_cache(self):
        ...
        if not saved_to_keyring and self._use_token_cache:
            ...
            try:
                ...
                # Create file with owner read/write only (0o600)
    ...
    async def aclose(self):
        ...
        await self._client.aclose()  # Close original httpx client
        await self._http.aclose()    # Close new AsyncHttpClient
    ...
    def prompt_authentication(self) -> None:
        ...
        # Get the frontend URL based on backend URL
        frontend_url = self._get_frontend_url()
        auth_url = f"{frontend_url}/login?sdk_auth=true"
        ...
        print(f"🌐 {auth_url}")
        print("\nAfter logging in, you'll get an API token.")
        print("Copy the token and paste it here:")
        print("="*60)
        ...
        # Prompt for token
        token = input("\n🔑 Paste your API token here: ").strip()
        ...
        if token:
            ...
            print("❌ No token provided. Operations requiring authentication will fail.")

    async def authenticate(self, token: Optional[str] = None, interactive: bool = True) -> bool:
        """Ensure the client has valid credentials loaded."""
        ...
        env_token = os.getenv("CYBERWAVE_API_KEY") or os.getenv("CYBERWAVE_TOKEN")
        if env_token:
    ...
    def _get_frontend_url(self) -> str:
        """Get the frontend URL based on the backend URL."""
        backend_base = self.base_url.replace('/api/v1', '')
    ...
    async def find_asset_by_registry_id(self, registry_id: str) -> Optional[Dict[str, Any]]:
        """Deprecated wrapper for :meth:`AssetsAPI.find_by_registry_id`."""

        warnings.warn(
            "Client.find_asset_by_registry_id is deprecated; use client.assets.find_by_registry_id instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return await self.assets.find_by_registry_id(registry_id)

    async def create_twin(
        self,
        payload: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Deprecated wrapper for :meth:`TwinsAPI.create`."""

        warnings.warn(
            "Client.create_twin is deprecated; use client.twins.create instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return await self.twins.create(payload)
    ...
    async def update_twin_state(
        ...
        *,
        ...
        rotation: Optional[Union[List[float], Dict[str, float]]] = None,
    ) -> Dict[str, Any]:
        """Deprecated wrapper for :meth:`TwinsAPI.update_state`."""

        warnings.warn(
            "Client.update_twin_state is deprecated; use client.twins.update_state instead.",
            DeprecationWarning,
            stacklevel=2,
        )
        return await self.twins.update_state(
            twin_uuid,
            position=position,
            rotation=rotation,
        )
    ...
    async def add_robot(
        ...
        """
        # DEPRECATION: The backend no longer exposes /robots.
        # Prefer using register_device_to_node(..., device_type='robot/...') and linking a Twin.
        logger.warning("add_robot() is deprecated. Use register_device_to_node with device_type='robot/...' and optional twin.")
    ...
    async def get_device(self, device_uuid: str) -> Dict[str, Any]:
        """Fetch a device details by UUID (includes capabilities, config, metadata)."""
        response = await self._request("GET", f"/devices/{device_uuid}")
    ...
    async def list_devices(
        ...
        """List devices with optional filters for device_type substring and capability key."""
        params: Dict[str, Any] = {}
        if device_type:
    ...
    async def patch_device_capabilities(
        self,
        device_uuid: str,
        capabilities: Optional[List[str]] = None,
        merge: bool = False,
        ...
        """
        Update device capabilities via PATCH.
        - If capabilities is provided and merge=False (default), replaces the list.
        - If merge=True, merges capabilities into existing.
        - Additionally supports explicit add/remove lists.
        """
        payload: Dict[str, Any] = {}
        ...
        response = await self._request("PATCH", f"/devices/{device_uuid}/capabilities", json=payload)
        return response.json()

    async def register_device_to_node(
        self,
        node_id: str,
        name: str,
        device_type: str,
        *,
        ...
        asset_registry_id: Optional[str] = None,
        environment_uuid: Optional[str] = None,
        project_uuid: Optional[str] = None,
    ) -> Dict[str, Any]:
        """
        Register a device on a node. If a `twin` block is provided containing
        `environment_uuid` and either `asset_uuid` or `asset_registry_id`,
        the backend will create and link a Twin.

        Example twin payload:
        {
          "environment_uuid": "...",
          "asset_uuid": "..." | null,
          "asset_registry_id": "so/arm100" | null,
          "name": "Device (Twin)",
          "description": "...",
        ...
        payload: Dict[str, Any] = {
            "name": name,
            "description": description or f"Device registered via SDK",
        ...
        if asset_registry_id:
            payload["asset_registry_id"] = asset_registry_id
        if environment_uuid:
            payload["environment_uuid"] = environment_uuid
        if project_uuid:
            payload["project_uuid"] = project_uuid
        response = await self._request("POST", f"/nodes/{node_id}/devices", json=payload)
        return response.json()

    async def link_device_twin(
        self,
        device_uuid: str,
        *,
        ...
        """
        Link an existing twin to a device or create a new twin and link it.

        Provide either `twin_uuid` or (`asset_uuid` | `asset_registry_id`).
        `environment_uuid` is optional; backend infers/creates when not provided.
        """
        payload: Dict[str, Any] = {}
        if twin_uuid:
            payload["twin_uuid"] = twin_uuid
        if asset_uuid:
            payload["asset_uuid"] = asset_uuid
        if asset_registry_id:
            payload["asset_registry_id"] = asset_registry_id
        if environment_uuid:
            payload["environment_uuid"] = environment_uuid
        ...
        response = await self._request("POST", f"/devices/{device_uuid}/twin", json=payload)
        return response.json()
    ...
    async def get_workspace_by_slug(self, slug: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if e.status_code == 404:
                detail_text = str(e.detail).lower() if e.detail is not None else ""
                if "workspace" in detail_text and "not found" in detail_text:
                    logger.info(f"Workspace with slug '{slug}' not found.")
                    return None

                logger.debug(
                    "Slug endpoint returned 404 without specific detail; attempting fallback lookup via workspaces list."
                )
                try:
                    workspaces = await self.get_workspaces()
                except Exception as fallback_err:
                    logger.warning(
                        "Fallback lookup for workspace slug '%s' failed: %s",
                        slug,
                        fallback_err,
                    )
                    return None
                ...
                for workspace in workspaces:
                    candidate_slug = workspace.get("slug")
                    if not candidate_slug:
                        metadata = workspace.get("metadata") or {}
                        candidate_slug = metadata.get("slug")
                    if candidate_slug == slug:
                        logger.info(
                            "Workspace with slug '%s' resolved via fallback lookup.",
                            slug,
                        )
                        return workspace
    ...
    async def get_workspaces(self) -> List[Dict[str, Any]]:
        ...
        logger.debug("Sending GET /users/workspaces.")
        try:
            response = await self._request( # Use _request helper
                method="GET",
    ...
    async def create_project(
        ...
        if not workspace_uuid and workspace_id is None:
            raise ValueError("Either workspace_uuid or workspace_id must be provided")
        ...
        logger.debug(f"Sending POST {endpoint}. Payload keys: {list(payload.keys())}")
        try:
            response = await self._request( # Use _request helper
                method="POST",
                url=endpoint,
            ...
            response_data = response.json()
            logger.info(
                "Project '%s' created successfully (UUID: %s) using endpoint %s.",
                name,
    ...
    async def get_projects(self, workspace_uuid: str) -> List[Dict[str, Any]]:
        """Retrieves projects and filters by the given workspace UUID."""
        headers = self._get_headers()
        if not self._access_token:
            raise AuthenticationError("Cannot get projects without an active session token.")

        endpoint = f"/projects"
        logger.debug(f"Sending GET {endpoint} and filtering by workspace {workspace_uuid}.")
        try:
            response = await self._client.get(endpoint, headers=headers)
            response.raise_for_status()
            projects = response.json()
            # Filter client-side by workspace UUID
            return [p for p in projects if str(p.get("workspace")) == str(workspace_uuid)]
        except httpx.HTTPStatusError as e:
    ...
    async def upload_level_definition(
        self, 
        project_id: int,
        level_definition: Dict[str, Any],  # Changed from LevelDefinition to Dict
    ) -> Dict[str, Any]:
        """
        Uploads a level definition to the backend.
        DEPRECATED: Use create_environment instead. Levels are now environments.
        
        Args:
            project_id: ID of the project to create the level in
            level_definition: The level definition dict (metadata and entities)
            
        Returns:
            The created level object as returned by the backend
        """
        warnings.warn(
            "upload_level_definition is deprecated. Use create_environment instead.",
            DeprecationWarning,
            stacklevel=2
        )
        ...
        # Extract data from the level definition dict for the backend
        metadata = level_definition.get('metadata', {})
        name = metadata.get('title', 'Imported Level')
        floor_number = metadata.get('floor_number', 0)
    ...
    async def _attempt_refresh(self) -> bool:
        ...
        # Knox tokens don't support refresh - user needs to login again
        logger.info("Knox tokens don't support refresh. User needs to login again.")
    ...
    async def login(self, username: Optional[str] = None, password: Optional[str] = None) -> None:
        ...
        try:
            token_data = None
            # Prefer modern generic auth endpoint when available
            try:
                # Generic auth under /api/v1/users/auth/login (aliased to /cli/login)
                cli_resp = await self._client.post(
                    "/users/auth/login",
                    json={"email": username, "password": password},
                )
                if cli_resp.status_code == 200:
                    token_data = cli_resp.json()
                else:
                    # Fall back to legacy auth endpoint
                    raise httpx.HTTPStatusError("CLI login not available", request=cli_resp.request, response=cli_resp)
            except (httpx.HTTPStatusError, httpx.RequestError):
                # Try dj-rest-auth (frontend-compatible) as secondary fallback
                try:
                    dj_resp = await self._client.post(
                        "/dj-rest-auth/login/",
                        json={"username": username, "email": username, "password": password},
                    )
                    if dj_resp.status_code == 200:
                        token_data = dj_resp.json()
                    else:
                        raise httpx.HTTPStatusError("dj-rest-auth not available", request=dj_resp.request, response=dj_resp)
                except (httpx.HTTPStatusError, httpx.RequestError):
                    # Final legacy OAuth-style fallback
            ...
            # The OAuth token endpoint returns ``access_token``/``refresh_token``.
            # Keep backwards compatibility with older responses that might use
            # ``token`` instead.
            self._access_token = (
            ...
            # Store additional session information (e.g. expires_in, token_type).
            # Do not duplicate the token fields themselves in this structure.
            self._session_info = {
    ...
    async def create_standalone_environment(
        self, 
        name: str, 
        description: str = "", 
        settings: Optional[Dict[str, Any]] = None,
        initial_assets: Optional[List[str]] = None
    ) -> Dict[str, Any]:
        """Create a standalone environment (not under a project)
        REFACTORED: Now delegates to specialized EnvironmentsAPI for proper segregation of competence
        
        Args:
            name: Environment name
            description: Environment description  
            settings: Environment settings
            initial_assets: List of asset registry IDs to add as digital twins (e.g., ['cyberwave/so101'])
        """
        # Delegate to specialized EnvironmentsAPI (proper architecture)
        return await self.environments.create_standalone(
            name=name,
            description=description,
            settings=settings,
    ...
    async def create_environment_link_share(self, environment_uuid: str, role_cap: str = "viewer") -> Dict[str, Any]:
        """Create a public link share for an environment
        REFACTORED: Now delegates to specialized EnvironmentsAPI for proper segregation of competence
        """
        # Delegate to specialized EnvironmentsAPI (proper architecture)
        return await self.environments.create_link_share(environment_uuid, role_cap)
