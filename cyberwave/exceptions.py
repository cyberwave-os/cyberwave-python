...
"""Cyberwave SDK exceptions — the one hierarchy the whole SDK raises and catches.

The OpenAPI generator emits a second copy of the ``ApiException`` family, which
made ``except UnauthorizedException`` depend on which module the author imported
from. ``python-sdk-gen.sh`` now rewrites ``cyberwave/rest/exceptions.py``
into a re-export of this module, so this file must keep defining every name the
generated client imports — dropping one breaks ``cyberwave.rest`` at import time.
"""  # noqa: E501
...
class ApiException(OpenApiException):
    ...
    def __str__(self):
        ...
        # request_headers is deliberately not rendered here: _handle_error
        # interpolates str(inner) into a CyberwaveAPIError that renders the same
        # dict, so both would print it. That is the single renderer; the value
        # stays on exc.request_headers.
...
class CyberwaveAPIError(CyberwaveError):
    ...
    Attributes:
        ...
        operation: The call the user made, stamped by
            ``BaseResourceManager._handle_error``; None outside a manager call
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.request_headers = request_headers
        # Declared so the attribute exists on every path, translated or not.
